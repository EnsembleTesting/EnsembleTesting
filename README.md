...
To address these challenges, we propose an ensemble-based test selection framework that introduces a statistically grounded scale unification mechanism to integrate multiple input-specific test selection metrics. To determine which metrics should be incorporated into the ensemble, we conduct a comprehensive empirical study evaluating existing metrics using two complementary criteria: the capability of fault detection and retraining guidance. Metrics that consistently demonstrate strong performance are then integrated into our framework.

We evaluate the proposed approach with two criteria across two Android malware datasets and two real-world autonomous driving datasets. Experimental results show that our method consistently outperforms both individual metrics and representative ensemble baselines, achieving statistically significant improvements under a wide range of out-of-distribution (OOD) scenarios. An ablation study further validates the contributions of the proposed scale unification and ensemble selection strategy. To support reproducibility and future research, our implementation is publicly available at GitHub.
