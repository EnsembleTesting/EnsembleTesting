'''
Driving models DAVE2V2
'''
import sys
sys.path.append('/driving')
...
class Dave2v2(Classifier):
    ...
    def graph(self, x_input,
        ...
        with tf.variable_scope("{}".format(name), reuse=reuse):
            ...
            for m in [conv1, conv2, conv3, conv4, conv5, dense1, dense2, dense3, dense4, dense5]:
                m.kernel_initializer = tf.initializers.truncated_normal(mean=0.0, stddev=0.1)
                m.bias_initializer = tf.initializers.zeros()
            return dense5, conv5, dense4, conv1, dense1
    ...
    def forward(self, x_tensor, y_tensor, reuse=False):
        ...
        """
        self.nn = self.graph
        softmax, self.conv5, self.dense4, self.conv1, self.dense1 = self.nn(
    ...
    def train(self, train_generator=None, val_xs=None, val_ys=None):
        ...
        with sess.as_default():
            ...
            for X_batch, y_batch in tqdm(train_generator):
                ...
                step_idx += 1
                if step_idx > 50000: # 1000*bs= number of samples used for training, 50000~ epoch=50
    ...
    def retrain(self, candidateX=None, candidatey=None, testX=None, testy=None, epochs=None):
        ...
        train_generator = data_generator(candidateX, candidatey,
                                     batch_size=self.hp_params.batch_size)
        print('******** num of imgs of training ******', candidateX.shape)
        # rebuild the graph
        tf.reset_default_graph()
        self.model_graph()
        cur_checkpoint = tf.train.latest_checkpoint(self.save_dir)
